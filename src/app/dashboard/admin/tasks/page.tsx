...
import { mockTasks, mockEmployees, mockProjects } from '../../../../utils/adminMockData';
...
export default function AdminTasks() {
  ...
  const cycleStatus = (id: string, currentStatus: string) => {
    ...
    setTasks(tasks.map(t => 
      t.id === id ? { ...t, status: statusMap[currentStatus] || 'Pending' } : t
    ));
  };
                  ...
                  <tr key={task.id} className="hover:bg-gray-50/50 transition-colors group">
                    ...
                    <td className="px-6 py-4 text-center">
                      <button 
                        ...
                        className={`flex items-center justify-between w-[110px] mx-auto px-3 py-1.5 rounded-md border text-[11px] font-bold ${getStatusColor(task.status)} hover:opacity-80 transition-opacity`}
                      >
                    ...
                    <td className="px-6 py-4 text-right">
                      <div className="flex justify-end gap-3 opacity-0 group-hover:opacity-100 transition-opacity">
                        ...
                        <button className="p-1.5 bg-gray-50 hover:bg-red-50 text-gray-400 hover:text-red-600 rounded transition-colors"><Trash2 className="w-3.5 h-3.5" /></button>
                      </div>
                    </td>
                  </tr>
                );
          ...
          <div className="bg-white rounded-xl shadow-xl w-[450px] border border-gray-200 overflow-hidden">
            ...
            <div className="p-6 space-y-4">
              <div>
                ...
                <input type="text" placeholder="e.g. Design Login Page" className="w-full px-3 py-2 bg-white border border-gray-200 rounded-lg text-[13px] focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500" />
              </div>
              <div>
